...
describe("Staking and Voting", function () {
    ...
    before(async function () {
        ...
        fixture = await createFixture(deployments, this, async (cmd) => {
            ...
            const [qEarly, qMid, qLate] = [1485119047619047619n, 1380952380952380952n, 1113095238095238095n]
    ...
    it("Should mint all shares in a 1:1 ratio at first", async function () {
        ...
        await expect(this.dao.connect(this.alice).mint(stakeAlice, this.dirk.address))
        ...
        await expect(this.dao.connect(this.carol).mint(stakeCarol, this.dirk.address))
            .to.emit(this.token, "Transfer")
    ...
    it("Should track votes according to stake", async function () {
        expect(await this.dao.votes(this.dirk.address)).to.equal(stakeAlice.add(stakeCarol))
        expect(await this.dao.votes(this.erin.address)).to.equal(stakeBob)
        expect(await this.dao.userVote(this.alice.address)).to.equal(this.dirk.address)
        expect(await this.dao.userVote(this.carol.address)).to.equal(this.dirk.address)
        expect(await this.dao.userVote(this.bob.address)).to.equal(this.erin.address)

        expect(await this.dao.totalSupply()).to.equal(total)
    })

    it("Should lock staked tokens", async function () {
        await expect(this.dao.connect(this.bob).burn(this.fred.address, burnBob)).to.be.revertedWith("Locked")
    })
    ...
    it("Should allow vote changes even for locked tokens", async function () {
        ...
        expect(await this.dao.userVote(this.bob.address)).to.equal(this.dirk.address)
    ...
    it("Should update vote totals on withdrawal", async function () {
        ...
        expect(await this.dao.votes(this.erin.address)).to.equal(stakeBob.sub(burnBob))
    })

    it("Should update vote totals on transfer", async function () {
        await expect(this.dao.connect(this.alice).transfer(this.bob.address, transferAliceBob))
            .to.emit(this.dao, "Transfer")
            .withArgs(this.alice.address, this.bob.address, transferAliceBob)

        expect(await this.dao.totalSupply()).to.equal(total.sub(burnBob))
        expect(await this.dao.votes(this.dirk.address)).to.equal(stakeAlice.add(stakeCarol).sub(transferAliceBob))
        expect(await this.dao.votes(this.erin.address)).to.equal(stakeBob.sub(burnBob).add(transferAliceBob))
    })
    ...
    it("Should set the operator to the elected address", async function () {
        ...
        expect((await this.dao.votes(this.dirk.address)).mul(2)).to.gt(await this.dao.totalSupply())
        expect(await this.dao.operator()).to.equal(this.bob.address)
        expect(await this.dao.pendingOperator()).to.equal(ZERO_ADDR)

        await expect(this.dao.setOperator(this.alice.address)).to.be.revertedWith("Not enough votes")
        ...
        await expect(this.dao.setOperator(this.erin.address)).to.be.revertedWith("Not enough votes")
        await expect(this.dao.setOperator(this.dirk.address)).to.be.revertedWith("Wait longer")
    ...
    it("Should remove operator-to-be if majority not held", async function () {
        ...
        await expect(this.dao.setOperator(this.erin.address)).to.be.revertedWith("Wait longer")
    ...
    describe("Tribute", function () {
        it("Should grow the token/share ratio", async function () {
            ...
            await expect(this.dao.connect(this.alice).burn(this.alice.address, stakeAlice))
                ...
                .to.emit(this.token, "Transfer")
                .withArgs(this.dao.address, this.alice.address, stakeAlice.mul(6).div(5))
        })
        ...
        it("Should return the same amount back* (rounding)", async function () {
            ...
            await expect(this.dao.connect(this.alice).burn(this.alice.address, sharesReceived))
                .to.emit(this.dao, "Transfer")
        ...
        it('Should "compound" profits', async function () {
            ...
            await expect(this.dao.connect(this.bob).burn(this.bob.address, withdrawBob))
                .to.emit(this.dao, "Transfer")
                .withArgs(this.bob.address, ZERO_ADDR, withdrawBob)
                .to.emit(this.token, "Transfer")
                .withArgs(this.dao.address, this.bob.address, withdrawBob.mul(156).div(100))
