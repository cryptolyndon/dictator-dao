...
const { createFixture, BN, e10, keccak256, toUtf8Bytes, abiCoder, solidityPack, incTime } = require("./framework")
...
describe("General functionality", function () {
    ...
    before(async function () {
        fixture = await createFixture(deployments, this, async (cmd) => {
            ...
            PERMIT_SIGNATURE_HASH = keccak256(toUtf8Bytes("Permit(address owner,address spender,uint256 value,uint256 nonce,uint256 deadline)"))
            const DOMAIN_SEPARATOR_HASH = keccak256(toUtf8Bytes("EIP712Domain(uint256 chainId,address verifyingContract)"))
            // TODO: Get chain ID
            DOMAIN_SEPARATOR = keccak256(abiCoder.encode(["bytes32", "uint256", "address"], [DOMAIN_SEPARATOR_HASH, 31337, this.dao.address]))
        })
    })
        ...
        it('Should accept a signed "permit" request', async function () {
            ...
            const dataHash = keccak256(
                abiCoder.encode(
                    ...
                    [PERMIT_SIGNATURE_HASH, this.alice.address, this.fred.address, halfEth, aliceNonce++, deadline]
                )
            )
            const digest = keccak256(solidityPack(["string", "bytes32", "bytes32"], ["\x19\x01", DOMAIN_SEPARATOR, dataHash]))
