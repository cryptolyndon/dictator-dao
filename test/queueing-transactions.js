...
const { createFixture, BN, e10, keccak256, abiCoder, incTime } = require("./framework")
...
describe("Queueing Transactions", function () {
    ...
    describe("Mechanism", function () {
        it("Should accept transactions from the operator", async function () {
            ...
            const data = []
            const hash = keccak256(abiCoder.encode(["address", "uint256", "bytes"], [target, value, data]))

            // Ensure a deterministic timestamp:
            const next = await incTime(1)

            await expect(this.dao.connect(this.bob).queueTransaction(target, value, data))
        ...
        it("Should only accept transactions from the operator", async function () {
            await expect(this.dao.connect(this.alice).queueTransaction(this.alice.address, BN(1_000_000), [])).to.be.revertedWith(
                "Operator only"
            )
        })
        ...
        it("Should require the operator to have a vote majority", async function () {
            ...
            await expect(this.dao.connect(this.bob).queueTransaction(this.dirk.address, BN(1_000_000), [])).to.be.revertedWith(
                "Not enough votes"
            )
        })
        ...
        it("Should let only the operator cancel", async function () {
            ...
            await expect(this.dao.connect(this.alice).cancelTransaction(target, value, data)).to.be.revertedWith("Operator only")

            await expect(this.dao.connect(this.bob).cancelTransaction(target, value, data))
        ...
        it("Should enforce a min delay before executing", async function () {
            ...
            await expect(this.dao.connect(this.bob).executeTransaction(target, value, data)).to.be.revertedWith("Too early")
        })
        ...
        it("Should enforce a max delay before executing", async function () {
            ...
            await expect(this.dao.connect(this.bob).executeTransaction(target, value, data)).to.be.revertedWith("Tx stale")
        })
        ...
        it("Should execute a scheduled transaction", async function () {
            ...
            // Not enough money
            await expect(this.dao.connect(this.bob).executeTransaction(target, value, data)).to.be.revertedWith("Tx reverted :(")

            // Send along enough money. Note that the tx is kept if it reverts:
            await expect(this.dao.connect(this.bob).executeTransaction(target, value, data, { value }))
        ...
        it("Should not execute an unscheduled transaction", async function () {
            ...
            // Unix epoch.
            await expect(this.dao.connect(this.bob).executeTransaction(this.bob.address, BN(1_000_000), [])).to.be.revertedWith("Tx stale")
        })
    })
    ...
    describe("Application - Example", function () {
        it("Should let the operator act on behalf of the DAO", async function () {
            ...
            const data = this.token.interface.encodeFunctionData("setMigrator", [this.fred.address])
            const hash = keccak256(abiCoder.encode(["address", "uint256", "bytes"], [target, value, data]))
            await incTime(1)
            await this.dao.connect(this.bob).queueTransaction(target, value, data)
            await incTime(DELAY)
