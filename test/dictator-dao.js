...
describe("DictatorDAO", function () {
    ...
    describe("Auction -- price", function () {
        ...
        it("Should reject at one past the above cutoff", async function () {
            ...
            await expect(this.token.buy(0, this.carol.address, { value: tooMuch })).to.be.revertedWith("Oversold")
        })
    })
    ...
    describe("Auction -- finalization", function () {
        ...
        it("Should cost in proportion to post-bonus amount", async function () {
            ...
            await expect(this.token.claimPurchase(0, this.alice.address))
                .to.emit(this.token, "Transfer")
                .withArgs(this.token.address, this.alice.address, BN(1_000_000).mul(qEarly).div(weekShares))

            await expect(this.token.claimPurchase(0, this.bob.address))
                .to.emit(this.token, "Transfer")
                .withArgs(this.token.address, this.bob.address, BN(1_000_000).mul(qMid).div(weekShares))
    ...
    describe("Staking and Voting", function () {
        ...
        it("Should give the first staker one share per token", async function () {
            ...
            await expect(this.dao.connect(this.alice).mint(stakeAmount, this.dirk.address))
                .to.emit(this.token, "Transfer")
            ...
            expect(await this.dao.balanceOf(this.alice.address)).to.equal(stakeAmount)
